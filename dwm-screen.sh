...
if [ $count == 3 ]; then
	...
	nvidia-settings --assign CurrentMetaMode="DP-0: nvidia-auto-select +4480+0 {rotation=left, ForceCompositionPipeline=On, ForceFullCompositionPipeline=On}, HDMI-0: nvidia-auto-select +1920+0 {ForceCompositionPipeline=On, ForceFullCompositionPipeline=On}"
elif [ $count == 2 ]; then
